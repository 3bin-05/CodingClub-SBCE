...
export default function ContactView({ settings }: ContactViewProps) {
  ...
  const socialCards = [
    ...
    { label: 'Instagram', value: socials.instagram ? (socials.instagram.split('/').pop() ? '@' + socials.instagram.split('/').pop() : 'Instagram') : '@sbce_codingclub', href: socials.instagram, icon: Instagram },
    { label: 'LinkedIn', value: 'SBCE Coding Club', href: socials.linkedin, icon: Linkedin },
    { label: 'GitHub Roster', value: socials.github ? (socials.github.split('/').pop() || 'GitHub') : 'sbce-codingclub', href: socials.github, icon: Github },
  ];
